...
namespace ortc_wrapper.Shared
{
    namespace Internal
    {
        ...
        {
            internal static string GenerateMediaSDP(string type, RTCRtpCapabilities capabilities, RTCIceGatherer iceGatherer, RTCDtlsTransport dtlsTransport, string ipAddress, IList<UInt32> streamSourceIds, string streamName, string mediaTrackLabel, string streamId)
            {
                string ret = null;
                ...
                if (capabilities != null)
                {
                    ...
                    string ssrc = SDPGenerator.GenerateSSRC(streamSourceIds, streamName  , mediaTrackLabel, streamId);
...
#warning TODO - a=fmtp:111 minptime=10; useinbandfec=1 SHOULD BE HANDLED BUT MINPTIME AND USEINBANDFEC ARE MISSING 
            ...
            internal static string GenerateSSRC(IList<UInt32> ssrcIds, string cnameSSRC, string audioSSRCLabel, string streamId)
            {
                string ret = null;
                if (ssrcIds.Count == 0 || string.IsNullOrEmpty(cnameSSRC) || string.IsNullOrEmpty(audioSSRCLabel) || string.IsNullOrEmpty(streamId))
                    return ret;
